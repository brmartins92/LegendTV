...
let getTitles = async movie => {
  ...
  const result = await page.evaluate(() => {
    let data = [];
    let countSeason = document.querySelectorAll("#slider-select-title > div > div > * ").length; 
    for(let i = 1; i <= countSeason; i++ ){
      let title = document.querySelector(`#slider-select-title > div > div > div:nth-child(${i}) > a > span`).innerText;
      let description = document.querySelector(`#slider-select-title > div > div > div:nth-child(${i}) > a > p`).innerText; 
      let id = document.querySelector(`#slider-select-title > div > div > div:nth-child(${i}) > a`).attributes['data-filme'].value; 
